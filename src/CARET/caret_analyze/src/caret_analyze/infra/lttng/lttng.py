...
class LiveEventCollection(Iterable, Sized):

    def __init__(self, live_msgs) -> None:
        self._iterable_events: IterableEvents

        self._iterable_events = MessageEventCollection(live_msgs)
        # if store_cache:
        #     self._store_cache(self._iterable_events, cache_path)
        # logger.info(f'Converted to {cache_path}')
...
class MessageEventCollection(IterableEvents):
    ...
    @cached_property
    def events(self) -> list[dict]:
        return self._to_dicts(self._live_msgs, self._size)

    def time_range(self) -> tuple[int, int]:
        return self._begin_time, self._end_time

    # @staticmethod
    def _to_dicts(self, live_msgs, count: int) -> list[dict]:
        events = []
        acceptable_tracepoints = set(Ros2Handler.get_trace_points())

        for msg in tqdm(live_msgs, total=count, desc='converting', mininterval=1.0):
            if type(msg) is not bt2._EventMessageConst:
                continue
            if msg.event.name not in acceptable_tracepoints:
                continue
...
class Lttng(InfraBase):
    ...
    def __init__(
        ...
        *,
        ...
        self._event_filters = [] if event_filters == None else event_filters
        self._data = Ros2DataModel()
        self._offset = int

        events, begin, end = self._parse_lttng_data(
            trace_dir_or_events,
            force_conversion,
            event_filters or [],
            store_events
        ...
        # self.data = data
        self._info = LttngInfo(self._data)
        self._source: RecordsSource = RecordsSource(self._data, self._info)
        self._counter = EventCounter(self._data, validate=validate)
        self.events = events
        self._begin = begin
        self._end = end
    ...
    def _parse_lttng_data(
        ...
        if isinstance(trace_dir_or_events, str):
            ...
            for event in event_collection:
                ...
                if event_name == 'ros2_caret:caret_init':
                    self._offset = Ros2Handler.get_monotonic_to_system_offset(event)
                    break

            handler = Ros2Handler(self._data, self._offset)
        ...
        elif isinstance(trace_dir_or_events, list): # Handling live events
            print('Collecting live events...')
            event_collection = LiveEventCollection(
                trace_dir_or_events)
            print('{} events found.'.format(len(event_collection)))
            ...
            if len(event_filters) > 0:
                print('filtered to {} events.'.format(filtered_event_count))
            print('Parsing live events finished.')
        else:
        ...
        return events_, begin, end

    def add_trace_data(
        self,
        trace_events: list[dict],
    ):
        if isinstance(trace_events, list): # Handling live events
            print('Collecting live events...')
            event_collection = LiveEventCollection(
                trace_events)
            print('{} events found.'.format(len(event_collection)))
            ...
            self._begin = min(self._begin, begin) if self._begin is not None else begin
            self._end = max(self._end, end) if self._end is not None else end
            
            common.start_time, common.end_time = self._begin, self._end

            handler = Ros2Handler(self._data, None)
            ...
            for event in tqdm(
                    ...
                    desc='loading',
                    mininterval=1.0):
                if len(self._event_filters) > 0 and \
                        any(not f.accept(event, common) for f in self._event_filters):
                    continue
                filtered_event_count += 1
                event_name = event[LttngEventFilter.NAME]
                handler_ = handler.handler_map[event_name]
        ...
        # Update information with new trace data
        self._info = LttngInfo(self._data)
        self._source = RecordsSource(self._data, self._info)
        self._counter = EventCounter(self._data, validate=True)
    ...
    def get_node_names_and_cb_symbols(
        ...
        """
        data_model_srv = DataModelService(self._data)
...
class LttngLiveMode(InfraBase):
    """
    Lttng data container class when using live mode.
    ...
    def _parse_lttng_data(
        ...
        if isinstance(trace_events, list): # Handling live events
            ...
            print('Type of trace_events:')
            print(type(trace_events))
            print('Collecting live events failed...')
