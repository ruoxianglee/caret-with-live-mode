...
def setup(
    *,
    ...
    # Session
    _create_session(session_name, full_path, lttng_live_mode)
...
def _create_session(
    ...
    if lttng_live_mode:
        try:
            # Use subprocess to execute the lttng create command
            # subprocess.run(["lttng", "create", session_name, "--live", "--output", full_path])
            subprocess.run(["lttng", "create", session_name, "--live"])
            print(f"Spawning a relayd daemon. Live session {session_name} created.")
        except subprocess.CalledProcessError as e:
            # Handle any errors that occurred during command execution
            print(f"Error occurred while creating the LTTng live session: {e}")
    else:
